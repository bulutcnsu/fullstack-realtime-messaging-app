...
const AuthForm= ({onAuthSuccess}) => { //onSelect ,getName,getToken
 ...
 if(name  === "name"){  setName(value);  
   if(value.length === 0){  
   setNameError(true); } else {setNameError(false)};}
...
//works properlyy
 ...
 if(name && password){
   ...
   console.log("handle sigin data", data);
    
   if (data.token) { 
     console.log("data token geldi");
  ...
  const handleLogin = async () => {
    ...
    const data = await res.json();
    console.log("bana gelen name",name);
     if (data.token){ onAuthSuccess(data.token,name);   
